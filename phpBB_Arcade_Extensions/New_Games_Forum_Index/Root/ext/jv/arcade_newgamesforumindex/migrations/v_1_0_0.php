...
namespace jv\arcade_newgamesforumindex\migrations;
...
class v_1_0_0 extends \phpbb\db\migration\migration
{
	...
	public function install_data()
	{
		...
		$configs = array(
			array('ext_newgames_forum_index_display', 1),
			array('ext_newgames_forum_index_num', 10)
		);
	...
	public function remove_data()
	{
		if ($this->db_tools->sql_table_exists("{$this->table_prefix}arcade_config"))
		{
			$sql = "DELETE FROM {$this->table_prefix}arcade_config
					WHERE " . $this->db->sql_in_set('config_name', array('ext_newgames_forum_index_display', 'ext_newgames_forum_index_num'));
